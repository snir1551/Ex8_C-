...
namespace WarGame {
    ...
    void Board::move(uint player_number, std::pair<int,int> source, MoveDIR direction)
    {
        ...
        if(targetRow >= board.size() || targetRow < 0 || targetCol >= board[0].size() || targetCol < 0 || board[targetRow][targetCol] != nullptr)
        {
    ...
    void Board::drawHealth() const
    {
       for(int row = 0; row < board.size(); row++)
       {
        ...
        {
          if(board[row][col] != nullptr)
            std::cout << "[" << board[row][col]->getHealth() << "]" << "[" << board[row][col]->getDamage() << "(" << board[row][col]->getNumPlayer() << ")" << "]" << " ";
          else
            std::cout << "[UUU][UU] ";
        }
        std::cout << std::endl;
    ...
    Soldier* Board::getCloseToEnemy(const Soldier* source) const
    {
        ...
        for(int row = 0; row < board.size(); row++)
        {
            for(int col = 0; col < board[0].size(); col++)
            {
                ...
                {
                    double distance = source->Distance(board[row][col], *this);
    ...
    std::vector<Soldier*> Board::getSoldierNear(const Soldier* source) const
    {
        ...
        for(int dirX : dirs)
        {
            for(int dirY : dirs)
            {
                ...
                {
                    if(locationSource.second + dirX >= 0 && locationSource.second + dirX < board[0].size() && locationSource.first + dirY >= 0 && locationSource.first + dirY < board.size())
                    {
                        if(board[locationSource.first + dirY][locationSource.second + dirX] != nullptr && board[locationSource.first + dirY][locationSource.second + dirX]->getNumPlayer() == board[locationSource.first][locationSource.second]->getNumPlayer())
                        {
                            vec.push_back(board[locationSource.first + dirY][locationSource.second + dirX]);
                        }
                    }
                }
            }
    ...
    Soldier* Board::mostCurrentHealth(const Soldier* source) const
    {
        ...
        for(int i = 0; i < board.size(); i ++)
        {
            for(int j = 0; j < board[0].size(); j++)
            {
                ...
                {
                    if(board[i][j]->getHealth() > 0 && (mostHealth == -1 || mostHealth < board[i][j]->getHealth()))
                    {
