...
void clear(Board& board, int boardWidth, int boardHeight)
{
...
TEST_CASE("Board Test")
{
    ...
    SUBCASE("move soldiers exceptions")
    {
        ...
        board[{9,9}] = new FootCommander(1);
        board[{9,0}] = new FootCommander(1);
        //CHECK_THROWS((board.move(1, {0,1}, Board::MoveDIR::Right))); // try to move null
        //CHECK_THROWS((board.move(1, {0,0}, Board::MoveDIR::Up))); // try to move one soldier on other soldier
        //CHECK_THROWS((board.move(1, {9,9}, Board::MoveDIR::Up))); // try to move out of the board
        //CHECK_THROWS((board.move(1, {9,9}, Board::MoveDIR::Right))); // try to move out of the board
        //CHECK_THROWS((board.move(1, {9,0}, Board::MoveDIR::Left))); // try to move out of the board
